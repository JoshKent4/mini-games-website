...
function drawWheel() {
    ...
    for (let i = 0; i < segments.length; i++) {
        ...
        let fontSize = 24; // Decrease the maximum text size
        ctx.font = `bold ${fontSize}px Arial`;
        let textWidth = ctx.measureText(segments[i]).width;
        while (textWidth > 150) { // Adjust the width constraint as needed
            fontSize--;
            ctx.font = `bold ${fontSize}px Arial`;
    ...
    ctx.closePath();
    ctx.fillStyle = '#000'; // Color of the triangle
    ctx.fill();
}
...
function displaySelectedSegment(segment) {
    ...
    ctx.fillStyle = '#000';
    ctx.font = 'bold 48px Arial'; // Big bold text
    ctx.textAlign = 'center';
...
function spinWheel() {
    ...
    function animateSpin() {
        ...
        if (progress < 1) {
            ...
            // Calculate the winning segment based on the closest center to the triangle point
            ...
            result.textContent = `${segments[winningSegment]} is the winner!!`;
